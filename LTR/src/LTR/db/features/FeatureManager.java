...
package LTR.db.features;

import LTR.db.DataPoint;
import LTR.db.DenseDataPoint;
import LTR.db.RankList;
import LTR.db.SparseDataPoint;
...
public class FeatureManager {
	...
	/**
	 * 从一个文件中读取样本，调用其多态方法，设置两个参数为 false
	 ...
	 * @param mustHaveRelDoc 是否一定要有相关文档
	 * @param useSparseRepresentation 特征是否稀疏表示
	 * @return
	 */
...
//					System.out.print("\rReading feature file [" + inputFile + "]: " + countRL + "... ");
	...
	/**
	 * 按照输入的文件中指定的特征id读取 特征id，每一个id占用一行的位置
	...
	/**
	 * 把数据集划分为k分，k-1分为训练集和1份为测试集，对每个训练集不会划分训练集和验证集，这里没有做随机划分，只是按照顺序划分，结果还回到参数中
	...
	/**
	 * 把数据集划分为k分，k-1分为训练集和1份为测试集，然后对每个训练集按照 tvs的比例划分训练集和验证集，这里没有做随机划分，只是按照顺序划分，结果还回到参数中
	...
	/**
	 * 把结果按照训练集和测试集划分 ，比例为 percentTrain
	...
	/**
	 * 在输入参数不正确的时候调用，作用为打印提示语
	 *@create_time：2015年1月4日下午4:28:29
	 *@modifie_time：2015年1月4日 下午4:28:29
