...
public class LambdaMART extends Ranker {
	...
	public void init()
	{
		...
		nThreshold = cf.getInt("threshold.candidates")<0?nThreshold:cf.getInt("threshold.candidates");
		nTrees = tree;
		int leaf = cf.getInt("leaf")<0?nTreeLeaves:cf.getInt("leaf");
		nTreeLeaves = leaf;
		float shrinkage = cf.getFloat("shrinkage");
		learningRate = shrinkage;
		int mls = cf.getInt("minLeafSupport")<0?minLeafSupport:cf.getInt("minLeafSupport");
		minLeafSupport = mls;
		nRoundToStopEarly = cf.getInt("estop");
		gcCycle = cf.getInt("gcc");
