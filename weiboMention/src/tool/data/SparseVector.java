...
public class SparseVector implements Iterable<VectorEntry>, Serializable {
	...
	/**
	 * vector add like a=a+b
	 */
	public void add(SparseVector vec){
		for(int idx:vec.getIndex()){
			check(idx);
			int i = getIndex(idx);
			data[i] = vec.get(idx);
		}
	...
	/**
	 * @return kl_diversity a given SparseVector vector
	 */
	public double KL_D(SparseVector vec) {
		double res = 0;
		for (int idx : this.getIndex()) {
			if (vec.contains(idx))
				res += get(idx) * Math.log(get(idx))-get(idx)*Math.log(vec.get(idx));
		}
		return res;
	}
