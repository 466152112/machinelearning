 ...
 * @class_describe锛�
 * @creator锛歾houge
 * @create_time锛�014骞�鏈�3鏃�涓婂�?:30:21
 * @modifier锛歾houge
 * @modified_time锛�014骞�鏈�3鏃�涓婂�?:30:21
 * @modified_note锛�
 * @version
 * 
 */
public class MathCal {
	 ...
	 * @modifie_time锛�014骞�鏈�3鏃�涓婂�?:30:28
	 */
	 ...
	 * @modifie_time锛�014骞�鏈�3鏃�涓婂�?:30:34
	 */
	 ...
	 * @param vectorQ
	 * @return
	 * @create_time�?014�?1�?日下�?:01:55
	 * @modifie_time�?014�?1�?�?下午6:01:55
	 */
	...
	public static double KLDistance(double[] vectorP, double[] vectorQ) {
		...
		if (vectorP.length != vectorQ.length) {
			System.out
					.println("error in the KLDistance function ,where the input vectors' length difference");
			System.exit(0);
		}
		double qsum = 0;
		...
		if (qsum > 1.00001) {
			System.out
					.println("error in the KLDistance function ,where the vectorQ sum vectors must small 1");
			System.exit(0);
		}
		double psum = 0;
		for (int i = 0; i < vectorQ.length; i++) {
		...
		if (psum > 1.00001) {
			System.out
					.println("error in the KLDistance function ,where the vectorP sum vectors must small 1");
			System.exit(0);
		}
		double result = 0;
		for (int i = 0; i < length; i++) {
			if (vectorP[i] > 1 || vectorP[i] < 0) {
				System.out
						.println("error in the KLDistance function ,where the input must be [0,1]");
				System.exit(0);
			}
			if (vectorP[i] > 0.001) {
	...
	/**
	 * @return kl_diversity given two SparseVector vector
	 */
	public static double KLDistance(SparseVector vectorP, SparseVector vectorQ) {

		double qsum = vectorQ.sum();
		double psum = vectorP.sum();
		if(qsum>1.01||psum>1.01){
			System.out.println("the distribution must be (0,1]" +psum+"\t"+qsum);
			System.exit(0);
		}
		double result = 0;

		for (int idx : vectorP.getIndex()) {
			if (vectorQ.contains(idx))
				result += vectorP.get(idx) *( Math.log(vectorP.get(idx))- Math.log(vectorQ.get(idx)));
		}
		if (result==0) {
	 ...
	 * @return kl_diversity given two SparseVector vector
	 */
	public static double KLDistance(DenseVector vectorP, DenseVector vectorQ) {
		assert vectorP.getSize()== vectorQ.getSize();
		...
		double result = 0;
		int size=vectorP.getSize();
		for (int i = 0; i < size; i++) {
			if (vectorP.get(i)!=0&&vectorQ.get(i)!=0) {
				result += vectorP.get(i) *( Math.log(vectorP.get(i))- Math.log(vectorQ.get(i)));
			}
		}
		if (result==0) {
			return Double.MAX_VALUE;
	...
	public static double cossim(double[] vector1, double[] vector2) {
		...
		if (vector1.length != vector2.length) {
			System.out
					.println("error in the cossim function ,where the input vectors' length difference");
			System.exit(0);
		}
		for (int i = 0; i < length; i++) {
		...
		if (denominator1 == 0 || denominator2 == 0) {
			System.out
					.println("error in the cossim function , where the denominator1 is zero");
			System.exit(0);
		}
	...
	public static double getMedia(List<? extends Number> num) {
