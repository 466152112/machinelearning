...
public class Sims {
	 ...
	 * @return Jaccard's coefficient
	 */
	public static<V extends Number> double jaccard(Set<V> uItems, Set<V> vItems) {
		int common = 0, all = 0;
	...
	/**
	 * AA's coefficient is defined as the number of common rated items of
	 * two users divided by the total number of their unique rated items.
	 * 
	 * @return AA's coefficient
	 */
	public static double adamic_Adar(Map<Long, Integer> uItems, Map<Long, Integer> vItems) {
		Map<Long, Integer> temp=new HashMap<>(uItems);
		temp.keySet().retainAll(vItems.keySet());
		double result=0;
		for (long key : temp.keySet()) {
			if (temp.get(key)!=0) {
				result+=1.0/Math.log(temp.get(key));
			}
		}
		return result;
	}
	...
	/**
	 * RA's coefficient is defined as the number of common rated items of
	 * two users divided by the total number of their unique rated items.
	 * 
	 * @return RA's coefficient
	 */
	public static double RA(Map<Long, Integer> uItems, Map<Long, Integer> vItems) {
		Map<Long, Integer> temp=new HashMap<>(uItems);
		temp.keySet().retainAll(vItems.keySet());
		double result=0;
		for (long key : temp.keySet()) {
			if (temp.get(key)!=0) {
				result+=1.0/temp.get(key);
			}
		}
		return result;
	}
