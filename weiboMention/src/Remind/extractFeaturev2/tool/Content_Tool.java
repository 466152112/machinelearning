...
public class Content_Tool extends Super_tool{

	final static double Max_KL = Double.MAX_VALUE;

	final static int topicNumber = 100;

	static Map<String, Integer> word_class_map = new HashMap<String, Integer>();
	static Map<String, Integer> word_index_map = new HashMap<String, Integer>();
	static User_profile_txt_Source user_profile=new User_profile_txt_Source();
	static Luence_txt_Source luencetooL=null;
	 ...
	 * 2、微博内容长度
	 * 3、是否包含短链接
	 * 4、有 几个 targetuser 的 followee参与了讨论
	...
	public List<Double> get_Content_Feature(User sourceUser,User targetUser,String content){
		...
		feature.add((double) filter.getShortLink(content).size());
		return feature;
	}
