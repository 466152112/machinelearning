...
public class extract_Feature_Main_in_pal {
	...
	public static void main(String[] arg) {
		...
		extract_Feature_Main_in_pal main = new extract_Feature_Main_in_pal();
	...
	public void Load_mention_weibo_cotent() {
		...
		try (BufferedWriter writer = FileIO.getWriter(LTR_Type_File
			...
			while ((oneLine = temp.readLine()) != null) {
				sourceList.add(oneLine);
				if (sourceList.size()>10000) {
					results.add(executor.submit(new getFeatureInPal(sourceList,weiboId_content)));
	...
	public synchronized void WriteResult(List<String> result){
		try {
			FileIO.writeList(LTR_Type_File.getSvmTypeInput(), result,true);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	class getFeatureInPal implements Callable<List<String>>{
		...
		Map<Long, String> weiboId_content;
		public  getFeatureInPal(List<String> sourceData,Map<Long, String> weiboId_content ) {
			this.sourceData=sourceData;
			this.weiboId_content=weiboId_content;
		}
		/* (non-Javadoc)
