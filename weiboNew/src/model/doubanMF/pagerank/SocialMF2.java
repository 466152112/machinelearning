...
public class SocialMF2 extends RecursiveTask<String>{
	...
	public void IterBySGD() {
		...
		while (iterCount < this.maxIter) {
			...
			if (result < 0.658) {
				...
				System.out.println("mf2 :"+alpeh +"\t"+ iterCount + "\tlearningRatio:"+learnRatio+"\tRMSE:" + result+"\t min RMSE:"+FinalRSME);
			}
		}
	...
	private double[] userGradient(long userid,int userIndex,long itemid,int itemIndex,double differenceRating,double[] pitemFeature,double[] puserFeature) {
		...
		if (friendPCC.get(userid)!=null) {
			for (long friendId : friendPCC.get(userid).keySet()) {
				...
				threeFeature=MatrixUtil.vectorAdd(threeFeature,  MatrixUtil.vectorMultiplyByFraction(userFeaturemap.get(friendId).copyFeature(),-alpeh*pcc));
				fenmu += pcc;
			}
		}
		gradient = MatrixUtil.vectorAdd(gradient, MatrixUtil.vectorMultiplyByFraction(puserFeature, theta1+alpeh*fenmu));
	...
	protected String compute() {
		this.IterBySGD();
		String result="SR2.pcc\t"+getFinalRMSE();
		return result;
	}
}
