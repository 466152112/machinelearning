...
public class SearchForRetweetWeibo {
	...
	// String target="";
	static String target = path + "retweet/retweetType.txt";
	static String sourcePath = path + "weibocontent/";
	...
	public static void main(String[] args) {
		...
		// read one By One
		for (int i = 151; i < readerFile.size(); i++) {
			int index=readerFile.get(i).lastIndexOf("/");
			if (index==-1) {
				 index=readerFile.get(i).lastIndexOf("\\");
			}
			String resultFileName=path+"retweet/"+readerFile.get(i).substring(index+1);
			...
			while ((temp = readWeiboUtil.readOnePairTweetInMen()) != null) {
				...
				if (!sourceIdSet.contains(temp.getWeiboId())) {
				...
				if (sourceWeiboTree.root!=null&&sourceWeiboTree.root.height==18) {
					...
					//write each time
					writeToFile(sourceWeiboTree.root,resultFileName);
	...
	// 中序遍历avl�?
	public static void writeToFile(AvlNode<OnePairTweet> t,String resultFileName) {
		if (t != null) {
			writeToFile(t.getLeft(),resultFileName);
			OnePairTweet temPairTweet=t.getElement();
			List<OnePairTweet> reTweets =temPairTweet .getRetweetList();
			writeUtil.writeOneLine(temPairTweet.toString(), resultFileName);
			if (reTweets != null && reTweets.size() >0) {
				for (OnePairTweet onePairTweet : reTweets) {
					writeUtil.writeOneLine(onePairTweet.toString(), resultFileName);
				}
			}
			writeUtil.writeOneLine("", resultFileName);
			//set the retweetList to null
			t.getElement().setRetweetList(null);
			
			writeToFile(t.getRight(),resultFileName);
		}
	}
