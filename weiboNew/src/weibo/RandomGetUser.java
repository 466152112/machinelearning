...
public class RandomGetUser {
	...
	String microblogDB;
	int microblogDBPort;
	public RandomGetUser(int randomSize, String path,String randomUserList,String randomUserWeibo,String socaiGraphDB,int socaiGraphDBPort,String microblogDB,int microblogDBPort) throws IOException {
	...
	private void getRandomUser(int randomNumber, String saveFileName) {
		...
		do {
			try{
				...
				idsSplit = ids.split(",");
				// 每次随机选取随机个
				for (int i = 0; i < idsSplit.length&& randUserList.size() < randomNumber&&!idsSplit[i].isEmpty(); i++) {
					String kk = idsSplit[i];
					/*
					 * 筛选条件：1、随机数小于2 2、不包含在已有的数据集randUserList中 3、用户集randUserList小于2
					 * 4、该用户含有至少10条微博 
					...
					currentBasic=new BasicDBObject("_id",Long.parseLong(kk.trim()));
					
						currentBasic=new BasicDBObject("user_id",
								Long.parseLong(kk.trim()));
						int currentNumber = microblogCollection.find(currentBasic)
								.count();
						if (currentNumber > 10 && randUserList.size() < randomNumber
								&& !randUserList.contains(kk.trim())) {
							randUserList.add(kk.trim());
							tempArrayList.add(kk.trim());
	...
	private static int commonFollow(String[] user1Follow,String user2Follow){
		int count=0;
		...
		for(int i=0,length1=user1Follow.length;i<length1;i++){
			for(int j=0,length2=idsSplit.length;j<length2;j++){
				if (user1Follow[i].equals(idsSplit[j])) {
					count++;
				}
			}
		}
