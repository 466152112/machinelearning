	...
	private double[] userGradient(long userid,int userIndex,long itemid,int itemIndex,double differenceRating,double[] pitemFeature,double[] puserFeature) {
		...
		if (friendPCC.get(userid)!=null) {
			for (long friendId : friendPCC.get(userid).keySet()) {
				...
				threeFeature=MatrixUtil.vectorAdd(threeFeature,  MatrixUtil.vectorMul(this.userFeaturemap.get(friendId).copyFeature(),pcc));
				fenmu += pcc;
			}
		}
		
		if (fenmu!=0) {
			threeFeature = MatrixUtil.vectorMul(threeFeature, -alpeh
					/ fenmu);
			gradient = MatrixUtil.vectorAdd(gradient, threeFeature);
		}
		gradient=MatrixUtil.vectorAdd(gradient, MatrixUtil.vectorMul(puserFeature, theta1+alpeh));
		return gradient;
	}
