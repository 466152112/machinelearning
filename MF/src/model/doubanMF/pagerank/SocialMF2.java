	...
	private double[] userGradient(long userid,int userIndex,long itemid,int itemIndex,double differenceRating,double[] pitemFeature,double[] puserFeature) {
		...
		if (friendPCC.get(userid)!=null) {
			for (long friendId : friendPCC.get(userid).keySet()) {
				...
				threeFeature=MatrixUtil.vectorAdd(threeFeature,  MatrixUtil.vectorMul(userFeaturemap.get(friendId).copyFeature(),-alpeh*pcc));
				fenmu += pcc;
			}
		}
		gradient = MatrixUtil.vectorAdd(gradient, MatrixUtil.vectorMul(puserFeature, theta1+alpeh*fenmu));
