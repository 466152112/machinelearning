...
public class BaseMF extends RecursiveTask<String> {
	...
	public BaseMF(int LatentDimension,
			...
			double learnRatio, double theta, double alpeh) {
	...
	public void IterBySGD() {
		...
		while (iterCount < this.maxIter) {
			...
			double result = this.getPrecision(this.testmatrix);
			System.out.println("the Precision in baseMF by " + iterCount
					+ "is :" + result);
	...
	private double[] userGradient(long userid, int userIndex, long itemid,
		...
		gradient = MatrixUtil.vectorAdd(gradient,
	 ...
	 *         ����1:37:27
	 */
	public double getPrecision(Map<Long, Map<Long, Double>> testMatrix) {
		...
		for (Long userid : userlisttest) {
			...
			List<Long> testItemidList = new ArrayList<>(testMatrix.get(userid)
					.keySet());
			...
			List<Long> itemidList = new ArrayList<>(itemMap.keySet());
			TwoValueComparatorInLong tvc = new TwoValueComparatorInLong(itemMap);
			Collections.sort(itemidList, tvc);
			itemidList = itemidList.subList(0, 10);
			testItemidList.retainAll(itemidList);
			result += testItemidList.size() * 1.0 / 10;
			count++;
		}
		result = result / count;
		return result;
