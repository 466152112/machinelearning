 ...
 * 
 */
public class SocialMF2 extends RecursiveTask<String> {
	...
	public SocialMF2(int LatentDimension,
		...
		System.out.println("enter the SocialMF2 Class--------------");
		...
		if (ratingMatrix == null) {
			System.out
					.println("error in SocialMF2 construction function: the ratingMatrix cannot be null");
			System.exit(0);
		}
	...
	public void IterBySGD() {
		...
		while (iterCount < this.maxIter) {
			...
			for (int userIndex = 0; userIndex < this.userSize; userIndex++) {
				...
				for (int itemIndex = 0; itemIndex < this.itemSize; itemIndex++) {
					...
					double[] puserFeature = userFeaturemap.get(userid)
							.copyFeature();
					predictRating = MatrixUtil.vectorMul(pitemFeature,
							puserFeature);
			...
			iterCount++;
			String FullLine="SocialReg2 Updating Epch " + iterCount + " Current full RMSE " + fullRMSE+ " Cold-Start RMSE "+coldRMSE ;
			WriteUtil<String> writeUtil=new WriteUtil<>();
			writeUtil.writeOneLine(FullLine, "douban_Five_"+trainSeq+"_Reg2_"+this.alpeh+"_result.txt");
			if (fullRMSE < FinalRSME) {
					FinalRSME = fullRMSE;
			} else {
				writeUtil.writeOneLine("Finish "+this.trainSeq+" "+ learnRatio+" "+ this.theta1+" "+ this.theta2+" "+ alpeh, "douban_Five_"+trainSeq+"Reg2_"+this.alpeh+"result.txt");
					break;
			}
		}
	...
	private double[] userGradient(long userid, int userIndex, long itemid,
		...
		double[] gradient = MatrixUtil.vectorMul(pitemFeature,
				differenceRating);
		...
		if (friendPCC.get(userid) != null) {
			for (long friendId : friendPCC.get(userid).keySet()) {
				...
				threeFeature = MatrixUtil.vectorAdd(threeFeature, MatrixUtil
						.vectorMul(userFeaturemap.get(friendId)
								.copyFeature(), -alpeh * pcc));
				fenmu += pcc;
			}
		}
		gradient = MatrixUtil.vectorAdd(
				gradient,
				MatrixUtil.vectorMul(puserFeature, theta1
						+ alpeh * fenmu));
	...
	private double[] itemGradient(long userid, int userIndex, long itemid,
			int itemIndex, double differenceRating, double[] pitemFeature,
			double[] puserFeature) {
		double[] gradient = MatrixUtil.vectorMul(puserFeature,
				differenceRating);
		double[] itemFeature1 = MatrixUtil.vectorMul(
				pitemFeature, theta2);
	...
	protected String compute() {
		this.IterBySGD();
		String result = "mf2 :" + alpeh + "\t" + this.trainSeq+ "\t min RMSE:" + FinalRSME;
		return result;
	}
}
