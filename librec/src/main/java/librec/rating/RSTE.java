		...
		for (int iter = 1; iter <= numIters; iter++) {
			...
			for (int u : trainMatrix.rows()) {
				...
				for (int f = 0; f < numFactors; f++) {
					for (int k : tks)
						sum_us[f] += tu.get(k) * User_factor.get(k, f);
				}
				...
				for (VectorEntry ve : trainMatrix.row(u)) {
					...
					double pred1 = DenseMatrix.rowMult(User_factor, u, Item_factor, j);
					double sum = 0.0;
					for (int k : tks)
						sum += tu.get(k) * DenseMatrix.rowMult(User_factor, k, Item_factor, j);
			...
			for (int u : socialMatrix.columns()) {
				...
				for (int p : bu.getIndex()) {
					...
					for (int j : pp.getIndex()) {
						...
						double pred1 = DenseMatrix.rowMult(User_factor, p, Item_factor, j);
						double sum = 0.0, ws = 0.0;
						for (int k : tps) {
							double tuk = tp.get(k);
							sum += tuk * DenseMatrix.rowMult(User_factor, k, Item_factor, j);
							ws += tuk;
						}
						double pred2 = ws > 0 ? sum / ws : 0;
						double pred = alpha * pred1 + (1 - alpha) * pred2;
						...
						for (int f = 0; f < numFactors; f++)
							PS.add(u, f, (1 - alpha) * csgd * Item_factor.get(j, f));
					}
				}
			}
