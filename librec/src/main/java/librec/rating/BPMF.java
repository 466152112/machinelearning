...
public class BPMF extends IterativeRecommender {
	...
	protected void buildModel() throws Exception {
		...
		User_factor.init(0, 1);
		Item_factor.init(0, 1);

		for (int f = 0; f < numFactors; f++) {
			mu_u.set(f, User_factor.columnMean(f));
			mu_m.set(f, Item_factor.columnMean(f));
		}

		DenseMatrix alpha_u = User_factor.cov().inv();
		DenseMatrix alpha_m = Item_factor.cov().inv();
		...
		for (int iter = 1; iter <= numIters; iter++) {
			...
			for (int f = 0; f < numFactors; f++)
				x_bar.set(f, User_factor.columnMean(f));
			S_bar = User_factor.cov();
			...
			for (int f = 0; f < numFactors; f++)
				x_bar.set(f, Item_factor.columnMean(f));
			S_bar = Item_factor.cov();
			...
			for (int gibbs = 0; gibbs < 2; gibbs++) {
				...
				for (int u = 0; u < numUsers; u++) {
					...
					for (int j : rv.getIndex()) {
						...
						for (int f = 0; f < numFactors; f++)
							MM.set(idx, f, Item_factor.get(j, f));
					...
					if (lam != null) {
						...
						for (int f = 0; f < numFactors; f++)
							User_factor.set(u, f, w1_P1_u.get(f));
					}
				}
				...
				for (int j = 0; j < numItems; j++) {
					...
					for (int u : jv.getIndex()) {
						...
						for (int f = 0; f < numFactors; f++)
							MM.set(idx, f, User_factor.get(u, f));
					...
					if (lam != null) {
						...
						for (int f = 0; f < numFactors; f++)
							Item_factor.set(j, f, w1_M1_j.get(f));
					}
				}
			} // end of gibbs
	...
	protected double predict(int u, int j) {
		return globalMean + DenseMatrix.rowMult(User_factor, u, Item_factor, j);
	}
