...
public class TrustSVD extends SocialRecommender {
	...
	protected void buildModel() throws Exception {
		for (int iter = 1; iter <= numIters; iter++) {
			...
			for (MatrixEntry me : trainMatrix) {
				...
				double pred = globalMean + bu + bj + DenseMatrix.rowMult(User_factor, u, Item_factor, j);
				...
				if (uv.getCount() > 0) {
					double sum = 0;
					for (int i : nu)
						sum += DenseMatrix.rowMult(Y, i, Item_factor, j);
				...
				if (tr.getCount() > 0) {
					double sum = 0.0;
					for (int v : tu)
						sum += DenseMatrix.rowMult(W, v, Item_factor, j);
			...
			for (MatrixEntry me : socialMatrix) {
				...
				double pred = DenseMatrix.rowMult(User_factor, u, W, v);
				double eut = pred - tuv;
	...
	protected double predict(int u, int j) {
		double pred = globalMean + userBias.get(u) + itemBias.get(j) + DenseMatrix.rowMult(User_factor, u, Item_factor, j);
