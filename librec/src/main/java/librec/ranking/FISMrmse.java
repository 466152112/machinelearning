...
public class FISMrmse extends IterativeRecommender {
		...
		for (int iter = 1; iter <= numIters; iter++) {
			...
			for (Cell<Integer, Integer, Double> cell : R.cellSet()) {
				...
				for (VectorEntry ve : Ru) {
					...
					if (i != j) {
						sum_ij += DenseMatrix.rowMult(User_factor, i, Item_factor, j);
						cnt++;
					}
				}
				...
				for (VectorEntry ve : Ru) {
					...
					if (i != j) {
						for (int f = 0; f < numFactors; f++) {
							double pif = User_factor.get(i, f);
							double delta = euj * wu * Item_factor.get(j, f) + regI * pif;
			...
			User_factor = User_factor.add(PS);
			Item_factor = Item_factor.add(QS);
	...
	protected double predict(int u, int j) {
		...
		for (VectorEntry ve : Ru) {
			...
			if (i != j) {
				sum += DenseMatrix.rowMult(User_factor, i, Item_factor, j);
				count++;
			}
		}
