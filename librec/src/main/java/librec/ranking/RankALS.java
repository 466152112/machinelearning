	...
	protected void buildModel() throws Exception {
		for (int iter = 1; iter < numIters; iter++) {
			...
			for (int j = 0; j < numItems; j++) {
				DenseVector qj = Item_factor.row(j);
				double sj = s.get(j);
			...
			for (int u : cus) {
				...
				for (VectorEntry ve : Ru) {
					...
					double rui = ve.get();
					// double cui = 1;
					DenseVector qi = Item_factor.row(i);
			...
			for (int u : cus) {
				...
				for (VectorEntry ve : Ru) {
					...
					sum_sr += sj * ruj;
					sum_cr += ruj;
					sum_cq = sum_cq.add(Item_factor.row(j));
				}
			...
			for (int i = 0; i < numItems; i++) {
				...
				for (int u : cus) {
					DenseVector pu = User_factor.row(u);
