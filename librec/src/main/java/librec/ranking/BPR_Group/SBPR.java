			...
			for (int s = 0, smax = numUsers * 100; s < smax; s++) {
				...
				if (SPu.size() > 0) {
					...
					for (int f = 0; f < numFactors; f++) {
						...
						double qif = Item_factor.get(i, f), qkf = Item_factor.get(k, f);
						double qjf = Item_factor.get(j, f);

						double delta_puf = cik * (qif - qkf) / (1 + suk) + ckj * (qkf - qjf);
						User_factor.add(u, f, lRate * (delta_puf + regU * puf));

						Item_factor.add(i, f, lRate * (cik * puf / (1 + suk) + regI * qif));

						double delta_qkf = cik * (-puf / (1 + suk)) + ckj * puf;
						Item_factor.add(k, f, lRate * (delta_qkf + regI * qkf));

						Item_factor.add(j, f, lRate * (cik * (-puf) + regI * qjf));
					...
					for (int f = 0; f < numFactors; f++) {
						...
						User_factor.add(u, f, lRate * (cij * (qif - qjf) + regU * puf));
						Item_factor.add(i, f, lRate * (cij * puf + regI * qif));
						Item_factor.add(j, f, lRate * (cij * (-puf) + regI * qjf));
