...
public abstract class IterativeRecommender extends Recommender {
	...
	protected DenseMatrix User_factor, last_User_factor;

	// factorized item-factor matrix
	protected DenseMatrix Item_factor, last_Item_factor;
	...
	protected double predict(int u, int j) {
		return DenseMatrix.rowMult(User_factor, u, Item_factor, j);
	}
...
//			updateLRate(iter);
	...
	protected void updates() {
		if (User_factor != null)
			last_User_factor = User_factor.clone();
		if (Item_factor != null)
			last_Item_factor = Item_factor.clone();
		if (userBias != null)
	...
	protected void undos(int iter) {
		...
		if (last_User_factor != null)
			User_factor = last_User_factor.clone();
		if (last_Item_factor != null)
			Item_factor = last_Item_factor.clone();
		if (last_UB != null)
	...
	protected void initModel() throws Exception {
		...
		User_factor = new DenseMatrix(numUsers, numFactors);
		Item_factor = new DenseMatrix(numItems, numFactors);

		// initialize model
		if (initByNorm) {
			User_factor.init(initMean, initStd);
			Item_factor.init(initMean, initStd);
		} else {
			User_factor.init(); // P.init(smallValue);
			Item_factor.init(); // Q.init(smallValue);
		}
	...
	protected void saveModel() throws Exception {
		...
		// write matrices P, Q
		FileIO.serialize(User_factor, dirPath + "userFactors" + suffix);
		FileIO.serialize(Item_factor, dirPath + "itemFactors" + suffix);
	...
	protected void loadModel() throws Exception {
		...
		// write matrices P, Q
		User_factor = (DenseMatrix) FileIO.deserialize(dirPath + "userFactors" + suffix);
		Item_factor = (DenseMatrix) FileIO.deserialize(dirPath + "itemFactors" + suffix);
